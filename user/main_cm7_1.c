...
int main(void)
{
    ...
    while(true)
    {
      if(IMU_Counter >= 10){
        ...
        imu660ra_acc_x -= 47.95;
        imu660ra_acc_y -= -0.51;
        imu660ra_acc_z -= (-2013.59 - 2048);
        
        if(E_flag <= 100 && E_flag >= 0){
          E_flag ++;
        } else if(E_flag > 100){
          E_flag = -1;
          if(imu660ra_gyro_x < -100){
            E = 110;
          }
        }
        imu660ra_gyro_x -= -2.19 - E;
        imu660ra_gyro_y -= -6.29;
        ...
        //printf("%.2f, %.2f, %.2f\r\n", Roll, Pitch, -Yaw);
        //printf("%d\r\n",imu660ra_gyro_x);
        ipc_send_data((uint32_t)((Roll + 180) * 100));
        ipc_send_data((uint32_t)((Yaw + 180) * 100));
        ipc_send_data((uint32_t)((Pitch + 180) * 100));
