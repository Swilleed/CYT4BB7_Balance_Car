...
uint8_t Tracking_Mode(uint8_t status)
{
    uint8_t mode = ON_LINE_TRACKING;
    if (status == 0x1F)
    {
        mode = OFF_LINE_TRACKING;
    }
...
void Tracking_Control(uint8_t status, int32_t baseSpeed)
{
    if (Tracking_Mode(status) == ON_LINE_TRACKING)
    {
        ...
        Motor_SetTargetSpeed(&motorLeft, speed1);
        Motor_SetTargetSpeed(&motorRight, speed2);
    }
    else
    {
        /*惯性导航*/
